...
class VarsModule(BaseVarsPlugin):
    ...
    def get_vars(self, loader, path, entities, cache=True):
        ...
        pwsafe_cli = pwsafe.PWSafeClient(sso_username, sso_password)
        project = pwsafe_cli.projects[pwsafe_project]
        pwsafe_vars = {}
        for cred in project.credentials:
            if cred.prerequisites != '':
                if cred.username != '':
                    key = "pwsafe_%s_username" % (cred.prerequisites)
                    value = cred.username if cred.username != '' else None
                    pwsafe_vars[key] = value
                if cred.password != '':
                    key = "pwsafe_%s_password" % (cred.prerequisites)
                    value = cred.password if cred.password != '' else None
                    pwsafe_vars[key] = value
